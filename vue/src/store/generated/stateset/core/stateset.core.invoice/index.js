...
const getDefaultState = () => {
    return {
        ...
        _Structure: {
            ...
            EventPaid: getStructure(EventPaid.fromPartial({})),
            EventVoided: getStructure(EventVoided.fromPartial({})),
    ...
    actions: {
        ...
        async QueryInvoiceAll({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.next_key != null) {
                    let next_values = (await queryClient.queryInvoiceAll({ ...query, 'pagination.key': value.pagination.next_key })).data;
        ...
        async QuerySentInvoice({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QuerySentInvoice', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QuerySentInvoiceAll({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.next_key != null) {
                    let next_values = (await queryClient.querySentInvoiceAll({ ...query, 'pagination.key': value.pagination.next_key })).data;
                    value = mergeResults(value, next_values);
                }
                commit('QUERY', { query: 'SentInvoiceAll', key: { params: { ...key }, query }, value });
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QuerySentInvoiceAll', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QueryTimedoutInvoice({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryTimedoutInvoice', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QueryTimedoutInvoiceAll({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.next_key != null) {
                    let next_values = (await queryClient.queryTimedoutInvoiceAll({ ...query, 'pagination.key': value.pagination.next_key })).data;
                    value = mergeResults(value, next_values);
                }
                commit('QUERY', { query: 'TimedoutInvoiceAll', key: { params: { ...key }, query }, value });
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryTimedoutInvoiceAll', payload: { options: { all }, params: { ...key }, query } });
        ...
        async sendMsgPayInvoice({ rootGetters }, { value, fee = [], memo = '' }) {
            try {
                ...
                if (e == MissingWalletError) {
                    throw new SpVuexError('TxClient:MsgPayInvoice:Init', 'Could not initialize signing client. Wallet is required.');
                }
                else {
                    throw new SpVuexError('TxClient:MsgPayInvoice:Send', 'Could not broadcast Tx: ' + e.message);
                }
            }
        },
        ...
        async sendMsgVoidInvoice({ rootGetters }, { value, fee = [], memo = '' }) {
            try {
                ...
                if (e == MissingWalletError) {
                    throw new SpVuexError('TxClient:MsgVoidInvoice:Init', 'Could not initialize signing client. Wallet is required.');
                }
                else {
                    throw new SpVuexError('TxClient:MsgVoidInvoice:Send', 'Could not broadcast Tx: ' + e.message);
                }
            }
        },
        ...
        async MsgPayInvoice({ rootGetters }, { value }) {
            try {
                ...
                if (e == MissingWalletError) {
                    ...
                    throw new SpVuexError('TxClient:MsgPayInvoice:Create', 'Could not create message: ' + e.message);
                }
            }
        },
        ...
        async MsgVoidInvoice({ rootGetters }, { value }) {
            try {
                ...
                if (e == MissingWalletError) {
                    ...
                    throw new SpVuexError('TxClient:MsgVoidInvoice:Create', 'Could not create message: ' + e.message);
                }
            }
        },
