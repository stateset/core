...
func TestRefundQuerySingle(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			desc:     "First",
			request:  &types.QueryGetRefundRequest{Id: msgs[0].Id},
			response: &types.QueryGetRefundResponse{Refund: msgs[0]},
		},
		{
			desc:     "Second",
			request:  &types.QueryGetRefundRequest{Id: msgs[1].Id},
			response: &types.QueryGetRefundResponse{Refund: msgs[1]},
		},
		{
			desc:    "KeyNotFound",
			request: &types.QueryGetRefundRequest{Id: uint64(len(msgs))},
